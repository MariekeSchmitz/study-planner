...
public class CompetenceValidator implements Validator{
    ...
    public void validate(Plan plan) {
    	...
    	for (Module module : moduleList) {
    		...
    		List<Course> courseList = module.getCourses();
			
			for (Course course : courseList) {
				
				for (Competence competence : course.getTaughtCompetences()) {
    	...
    	// für alle Module checken, ob benötige Kompetenzen nicht in höherem Semester liegen
    	for (Module module : moduleList) {
    	
    		int currentSemester = module.getSemesterCurrent();
    		
    		for (Competence competence : module.getNeededCompetences()) {				// wenn Competence nicht erkannt, dann Strings reinpacken und über Competence-Namen reinpacken
    		...
    		for (Module module : moduleMap.get(semester)) {
    			...
    			for (int i = 1; i < semester; semester++) {
    				...
    				for (Module tempModule : tempList) {
    					...
    					List<Course> courseList = tempModule.getCourses();
    					
    					for (Course course : courseList) {
    						taughtCompetences.addAll(course.getTaughtCompetences());
    					}
